...
class Encryptor(EasyFrame):
	...
	def Encrypt(self):
		...
		# Helping function to dtoxy(), it is needed as the Hilbert curve's individual parts are rotated with respect to each other.
		def rot(n,x,y,rx,ry):
			if ry == 0:
				if rx == 1:
					x = n-1 - x
		...
		# Function which generates a key using the password, which is used to determine the starting point of encoding.
		def keygen(n):
			key = 0		
		...
		#This is when an image file is selected
			...
			for index in range(len(message)):
				ch=message[index]
				asc=ord(ch)
				# asc_bin is a 9-digit binary number which is encoded in 3 pixels. The last digit
